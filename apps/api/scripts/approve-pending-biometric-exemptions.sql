-- Idempotent production repair for biometric exemptions.
-- All pending rows become APPROVED. To respect the existing one-active-row-per-
-- target indexes, only the newest pending row is activated when duplicates
-- exist, and none is activated when that target already has an active row.
...
-- Verification: pending_rows should be zero. Approved inactive rows are kept
-- when activating them would violate the one-active-exemption-per-target rule.
SELECT
  COUNT(*) FILTER (WHERE "status" = 'PENDING_SUPERVISOR') AS pending_rows,
  COUNT(*) FILTER (WHERE "status" = 'APPROVED' AND "is_active" = true) AS approved_active_rows,
  COUNT(*) FILTER (WHERE "status" = 'APPROVED' AND "is_active" = false) AS approved_inactive_rows
